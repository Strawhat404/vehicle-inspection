...
docker compose exec -T backend sh -c 'NODE_PATH=/app/node_modules node --test /unit_tests/*.test.js'

echo "[2/2] Running API integration tests"
docker compose exec -T backend sh -c 'NODE_PATH=/app/node_modules node --test /API_tests/*.test.js'
